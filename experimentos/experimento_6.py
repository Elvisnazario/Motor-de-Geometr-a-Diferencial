...
# Importamos el pipeline del experimento anterior tirando de su modularidad estándar
from experimentos.colapso_detenido_v1_9 import ejecutar_pipeline_colapso
...
def ejecutar_barrido_robustez():
    ...
    masas_prueba = [
        ...
        1.0e31,       # 5 Masas Solares (Agujero negro ligero)
        1.0e33        # 500 Masas Solares (Agujero negro intermedio)
    ]
    ...
    for m in masas_prueba:
        for a in alphas_prueba:
            try:
                # El pipeline v1.9 calcula y exporta silenciosamente sus CSV/PNG/JSON individuales
                res = ejecutar_pipeline_colapso(masa=m, alpha=a, resolucion=4000)
                
                # Extraemos los observables clave para el análisis estadístico
                ...
                # Si una combinación diverge, se registra pero no se detiene el experimento global
                tabla_resultados.append({
                    "Masa (kg)": f"{m:.2e}",
                    "Alpha": f"{a:.2e}",
                    "R_eq (m)": "DIVERGENCIA",
    ...
    # 3. Persistencia de la tabla de robustez global en un reporte JSON estructurado
